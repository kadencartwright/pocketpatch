...
const configSetBindAddressCommand = (
  env: ConfigEnv
) =>
  Command.make(
    "set-bind-address",
    ...
    ({ bindAddress }) =>
      Effect.gen(function*() {
        const configService = yield* ConfigService;
        const config = yield* configService.load(env);
        const nextConfig = yield* configService.setBindAddress(config, bindAddress);
